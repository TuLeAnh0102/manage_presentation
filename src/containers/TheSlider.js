...
export default class SlideshowGallery extends React.Component {
    ...
    */
    componentDidMount() {
        ...
        this.containerElm = this.rootElm.querySelector(".slideshow-container");
    ...
    */
    ...
    handleTypeContent = (input, index) => {
        if (input['typeContent']){
            switch (input.typeContent) {
                case "text":
                    return (
                        <div
                            key={index}
                        ...
                        >
                            <div className="content-text">
                                {ReactHtmlParser(input.description)}
                            </div>
                        </div>
                    )
                case "iframe":
                    return(
                        ...
                        >
                            <div className="content-iframe">
                                <iframe src={input.description} frameBorder="0" allowFullScreen style={{position: 'absolute', border: 'none'}}>
                                </iframe>
                            </div>
                        </div>
                    )
                ...
                <div className="slideshow-container">
                    {
                        this.props.input.map((item, index) => {
                            return this.handleTypeContent(item, index);
                        })
                    }

                    <button className="prev" onClick={this.backward}>❮</button>
                    <button className="next" onClick={this.forward}>❯</button>
                </div>
            </div>
        );
    }
